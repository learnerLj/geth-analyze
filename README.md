...
以太坊源码解读，主要为智能合约方向，关注底层合约的执行机制。

源码解读时只对部分细节做出解释并且保留了英文注释，对于容易理解的英文注释并未翻译成中文。

源码解读所需要的理论基础存放在 `analyzeSourceCode` 文件夹内，未来将会越来越系统。

欢迎修正、共同在源码上以注释的形式解读，并且在  `analyzeSourceCode` 内提供您写的理论分析的文章。
